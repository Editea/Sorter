...
class Editea_Sorter_Block_Adminhtml_Sorter_Edit_Tab_Form extends Mage_Adminhtml_Block_Widget_Form
{
    protected function _prepareForm()
    {
        ...
        $this->setForm($form);
        $fieldset = $form->addFieldset('sorter_form',
            ['legend' => Mage::helper('sorter')->__('Item information')]);


        $fieldset->addField('editea_report_id', 'text', [
            'label' => Mage::helper('sorter')->__('Id'),
            'name' => 'editea_report_id',
            'readonly' => true,
        ]);

        $fieldset->addField('report_type', 'text', [
            'label' => Mage::helper('sorter')->__('Type'),
            'name' => 'report_type',
            'readonly' => true,
        ]);

        $fieldset->addField('report_level', 'text', [
            'label' => Mage::helper('sorter')->__('Level'),
            'name' => 'report_level',
            'readonly' => true,
        ]);

        $fieldset->addField('report_item_id', 'text', [
            'label' => Mage::helper('sorter')->__('Item Id'),
            'name' => 'report_item_id',
            'readonly' => true,
        ]);

        $fieldset->addField('report_message', 'text', [
